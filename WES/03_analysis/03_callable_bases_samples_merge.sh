...
VALIDATOR="${HF_SCC_ROOT}/WES/03_analysis/03_validate_callable_rows.r"
CONTRACT="${HF_SCC_ROOT}/WES/01_source/wes_callable_contract.tsv"
RUN_CONTRACT="${BASE_DIR}/02_data/02_processed/callable_bases/run_contract.tsv"
R_BIN="${R_BIN:-R}"

CALLABLE_COLUMN=$("$R_BIN" --vanilla --slave -f "$VALIDATOR" --args \
...
{
  printf "sample_name\ttotal_reads_mapq20\t%s\tmapq\tbq\n" "$CALLABLE_COLUMN"
  cat "${rows[@]}" | sort -V
} > "$temporary"
